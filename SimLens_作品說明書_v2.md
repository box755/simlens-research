...
3. **時序對齊**：系統依據時間軸將 LLaVA-NeXT 的視覺段描述與 Whisper 的轉錄時間戳進行對齊，把屬於同一時段的視覺描述與語音內容組合成「結構化文字片段」。這個方法借鏡了 UMaT（Unified Multi-modal as Text）框架的設計理念，將多模態資訊統一為 LLM 友善的文字表示。
